...
Run with::
    ...
    just test-hardware packages/brother_ptouch_driver/tests/hardware/test_status.py
...
from brother_ptouch_driver import discover_printers, query_status, select_printer
...
def test_status_cli_command():
    """brother-ptouch-driver status renders a human-readable status table."""
    _require_printers()
    runner = CliRunner()
...
def test_discover_status_cli_flag():
    """brother-ptouch-driver discover --status renders status tables per printer."""
    _require_printers()
    runner = CliRunner()
