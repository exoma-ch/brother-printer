...
| `just gen-test-images` | Regenerates committed PNG fixtures under `tests/hardware/assets/`. |
...
(all `qr_*`, `gray_*`, and `distort_100.png` must exist under `tests/hardware/assets/`).
...
All opt-in hardware tests live under `tests/hardware/`:
...
| `distort_100.png` | 100×100 px; non-integer scale to every tape width for `allow_distortion=True` |
