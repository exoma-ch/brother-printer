...
def test_print_image_selects_printer_by_identifier(
    ...
    with patch(
        "brother_ptouch_driver.printing.image_to_raster",
        return_value=[b"\x00" * 70] * 60,
    ):
        with patch("brother_ptouch_driver.printing.encode_job", return_value=b"job"):
            print_image(
...
def test_print_image_raises_tape_mismatch(
    ...
    from brother_ptouch_driver import TapeMismatchError, print_image
...
def test_print_image_raises_when_no_tape_loaded(
    ...
    from brother_ptouch_driver import PrinterNotReadyError, print_image
...
def test_select_printer_returns_first_when_identifier_none():
    ...
    from brother_ptouch_driver import select_printer
...
def test_query_status_reads_status_reply(mock_transport_cls, mock_decode_status):
    ...
    from brother_ptouch_driver import query_status
...
def test_print_image_raises_half_cut_on_non_laminated(
    ...
    from brother_ptouch_driver import HalfCutNotSupportedError, print_image
...
def test_print_strip_raises_half_cut_on_non_laminated(
    ...
    from brother_ptouch_driver import HalfCutNotSupportedError, print_strip
...
@patch("brother_ptouch_driver.printing.UsbTransport")
@patch("brother_ptouch_driver.printing.discover")
def test_print_png_decodes_bytes_and_prints(
    mock_discover,
    mock_transport_cls,
    mock_status_request,
    mock_decode_status,
    ...
    """print_png() decodes PNG bytes and delegates to print_image()."""
    from io import BytesIO

    from brother_ptouch_driver import print_png
    ...
    image = Image.new("L", (TapeWidth.MM_24.print_area_pins, 80), 255)
    buffer = BytesIO()
    image.save(buffer, format="PNG")
    png_bytes = buffer.getvalue()

    written = print_png(png_bytes, TapeWidth.MM_24, scale=True)

    mock_image_to_raster.assert_called_once()
    assert mock_image_to_raster.call_args.kwargs["scale"] is True
    assert written == 100
